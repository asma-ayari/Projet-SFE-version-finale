...
export class CoursDetail implements OnInit, OnDestroy {
  ...
  translatingDynamic = signal(false);
  ...
  // Math quiz specific signals (ID: 7 - Page 2)

  mathQuizPhase = signal<'idle' | 'countdown' | 'question' | 'waiting' | 'finished'>('idle');

  mathCountdownValue = signal<number>(10);

  mathTrialCount = signal<number>(0);

  mathCurrentEquation = signal<string>('');

  mathCorrectAnswer = signal<number>(0);

  mathUserAnswer = signal<number | null>(null);

  mathReactionTimes = signal<number[]>([]);

  mathStartTime = signal<number>(0);

  mathScore = signal<number>(0);

  mathFeedback = signal<{ isCorrect: boolean; messageKey: string; answer?: number } | null>(null);

  mathTransitionCountdown = signal<number>(0);

  private mathCountdownInterval: any = null;

  private mathTransitionInterval: any = null;
  ...
  isDynamicCourse = signal<boolean>(false);

  dynamicContentHtml = signal<SafeHtml | null>(null);

  dynamicCoverUrl = signal<string>('');

  courseListRoute = computed(() => this.router.url.startsWith('/formateur/') ? '/formateur/cours' : '/apprenant/cours');
  ...
  private coursePageMap = { 1: 6, 2: 7, 3: 5, 4: 6, 5: 6, 6: 5, 7: 10, 8: 5, 9: 5, 10: 6, 11: 7 };
  ...
  ngOnInit() {
    ...
    if (id) {
      ...
      if (!this.staticCourseIds.has(courseId)) {
        this.isDynamicCourse.set(true);
      }

      this.loadCourseFromApi(courseId);

      this.langSub = this.translate.onLangChange.subscribe(() => {
        if (this.isDynamicCourse() && !this.router.url.startsWith('/formateur/')) {
          this.loadCourseFromApi(courseId, true);
        }
      });
  ...
  private loadCourseFromApi(courseId: number, isLangReload = false): void {
    const isFormateurRoute = this.router.url.startsWith('/formateur/');

    if (isLangReload && this.isDynamicCourse()) {
      this.translatingDynamic.set(true);
    }

    const lang = this.languageService.getCurrentLang();
    const courseRequest = isFormateurRoute
      ? this.coursService.manageGet(courseId)
      : this.coursService.getPublished(courseId, lang);

    courseRequest.pipe(
      catchError((err) => {
        if (err.status === 404) {
          this.router.navigate([isFormateurRoute ? '/formateur/cours' : '/apprenant/cours']);
        } else {
          this.loadingError.set(true);
          this.errorMessage.set('Erreur lors du chargement du cours. Veuillez réessayer.');
        }
        return of(null);
      })
    ).subscribe({
      next: (course) => {
        this.translatingDynamic.set(false);
        if (course) {
          this.applyCourseMetadata(course, courseId);
        }
      },
      error: () => {
        this.translatingDynamic.set(false);
  ...
  private applyCourseMetadata(course: CourseDetail, courseId: number): void {
    this.courseTitle.set(course.title);

    if (this.isDynamicCourse()) {
      const body = (course.content || course.description || '').trim();
      this.courseTotalPages.set(1);
      this.currentPage.set(1);
      this.dynamicContentHtml.set(
        this.sanitizer.bypassSecurityTrustHtml(
          this.normalizeDynamicContent(body || 'Aucun contenu disponible pour ce cours.')
        )
      );
      this.dynamicCoverUrl.set(
        course.image_url ? this.coursService.resolveImageUrl(course.image_url) : ''
      );
    } else {
  ...
  private normalizeDynamicContent(content: string): string {
  ...
  getRoadConditionLabel(): string {
    ...
    switch (this.selectedRoadCondition()) {
      ...
      case 'wet': return this.translate.instant('COURSE_DETAIL_ID4.PAGE_5_ROAD_WET');

      case 'soaked': return this.translate.instant('COURSE_DETAIL_ID4.PAGE_5_ROAD_SOAKED');

      default: return this.translate.instant('COURSE_DETAIL_ID4.PAGE_5_ROAD_DRY');
  ...
  getVisualFieldDescription(): string {
    ...
    if (speed <= 40) {
      ...
      return 'COURSE_DETAIL_ID5.PAGE_5_SPEED_DESC_LOW';

    }

    if (speed <= 70) {

      return 'COURSE_DETAIL_ID5.PAGE_5_SPEED_DESC_MEDIUM';

    }

    if (speed <= 100) {

      return 'COURSE_DETAIL_ID5.PAGE_5_SPEED_DESC_HIGH';

    }

    return 'COURSE_DETAIL_ID5.PAGE_5_SPEED_DESC_VERY_HIGH';
  ...
  getSpeedLimitForEnvironment(): { environment: string; limit: number; reason: string }[] {
    ...
    return [
      ...
      {
        environment: 'COURSE_DETAIL_ID5.PAGE_6_ENV_CITY',
        limit: 50,
        reason: 'COURSE_DETAIL_ID5.PAGE_6_REASON_CITY'
      },

      {
        environment: 'COURSE_DETAIL_ID5.PAGE_6_ENV_ROAD',
        limit: 80,
        reason: 'COURSE_DETAIL_ID5.PAGE_6_REASON_ROAD'
      },

      {
        environment: 'COURSE_DETAIL_ID5.PAGE_6_ENV_EXPRESSWAY',
        limit: 110,
        reason: 'COURSE_DETAIL_ID5.PAGE_6_REASON_EXPRESSWAY'
      },

      {
        environment: 'COURSE_DETAIL_ID5.PAGE_6_ENV_HIGHWAY',
        limit: 130,
        reason: 'COURSE_DETAIL_ID5.PAGE_6_REASON_HIGHWAY'
      }
  ...
  getBottleDisplayName(bottle: string): string {
    ...
    const labels: { [key: string]: string } = {
      ...
      wine: 'COURSE_DETAIL_ID6.BOTTLE_WINE',

      beer: 'COURSE_DETAIL_ID6.BOTTLE_BEER',

      whisky: 'COURSE_DETAIL_ID6.BOTTLE_WHISKY',

      champagne: 'COURSE_DETAIL_ID6.BOTTLE_CHAMPAGNE',

      cocktail: 'COURSE_DETAIL_ID6.BOTTLE_COCKTAIL',

      vodka: 'COURSE_DETAIL_ID6.BOTTLE_VODKA',

      rum: 'COURSE_DETAIL_ID6.BOTTLE_RUM',

      'beer-strong': 'COURSE_DETAIL_ID6.BOTTLE_BEER_STRONG'
  ...
  getDrinkEquivalences(): {
    id: string;
    descriptionKey: string;
    volumeKey: string;
    degree: string;
    doses: number;
    iconType: 'beer' | 'cocktail' | 'wine' | 'whisky';
  }[] {
    return [
      { id: 'beer-light', descriptionKey: 'COURSE_DETAIL_ID6.DRINK_BEER_LIGHT', volumeKey: 'COURSE_DETAIL_ID6.VOLUME_33', degree: '5.9°', doses: 2, iconType: 'beer' },
      { id: 'beer-strong', descriptionKey: 'COURSE_DETAIL_ID6.DRINK_BEER_STRONG_LABEL', volumeKey: 'COURSE_DETAIL_ID6.VOLUME_50', degree: '10°', doses: 4, iconType: 'beer' },
      { id: 'alcopop', descriptionKey: 'COURSE_DETAIL_ID6.DRINK_COCKTAIL', volumeKey: 'COURSE_DETAIL_ID6.VOLUME_25', degree: '5°', doses: 1, iconType: 'cocktail' },
      { id: 'wine', descriptionKey: 'COURSE_DETAIL_ID6.DRINK_WINE', volumeKey: 'COURSE_DETAIL_ID6.VOLUME_75', degree: '12°', doses: 7, iconType: 'wine' },
      { id: 'whisky', descriptionKey: 'COURSE_DETAIL_ID6.DRINK_WHISKY', volumeKey: 'COURSE_DETAIL_ID6.VOLUME_9', degree: '40°', doses: 3, iconType: 'whisky' },
    ];
  }

  getCurrentDrinkBySlider() {
  ...
  getBrainProcessingSteps(): { step: number; icon: string; titleKey: string; descriptionKey: string }[] {

    return [

      { step: 1, icon: 'fa-eye', titleKey: 'COURSE_DETAIL_ID7.BRAIN_STEP_1_TITLE', descriptionKey: 'COURSE_DETAIL_ID7.BRAIN_STEP_1_DESC' },

      { step: 2, icon: 'fa-brain', titleKey: 'COURSE_DETAIL_ID7.BRAIN_STEP_2_TITLE', descriptionKey: 'COURSE_DETAIL_ID7.BRAIN_STEP_2_DESC' },

      { step: 3, icon: 'fa-hand-pointer', titleKey: 'COURSE_DETAIL_ID7.BRAIN_STEP_3_TITLE', descriptionKey: 'COURSE_DETAIL_ID7.BRAIN_STEP_3_DESC' }
  ...
  getFactorsAffectingReaction(): { factorKey: string; icon: string }[] {

    return [

      { factorKey: 'COURSE_DETAIL_ID7.FACTOR_FATIGUE', icon: 'fa-bed' },

      { factorKey: 'COURSE_DETAIL_ID7.FACTOR_ALCOHOL', icon: 'fa-wine-glass' },

      { factorKey: 'COURSE_DETAIL_ID7.FACTOR_PHONE', icon: 'fa-mobile-alt' },

      { factorKey: 'COURSE_DETAIL_ID7.FACTOR_MEDS', icon: 'fa-pills' },

      { factorKey: 'COURSE_DETAIL_ID7.FACTOR_AGE', icon: 'fa-user-clock' }
  ...
  getCannabisEffects(): { effect: string; icon: string; descriptionKey: string }[] {

    return [

      { effect: 'perception', icon: 'fa-eye-slash', descriptionKey: 'COURSE_DETAIL_ID9.EFFECT_PERCEPTION' },

      { effect: 'attention', icon: 'fa-brain', descriptionKey: 'COURSE_DETAIL_ID9.EFFECT_ATTENTION' },

      { effect: 'reaction', icon: 'fa-clock', descriptionKey: 'COURSE_DETAIL_ID9.EFFECT_REACTION' },

      { effect: 'coordination', icon: 'fa-hands', descriptionKey: 'COURSE_DETAIL_ID9.EFFECT_COORDINATION' },

      { effect: 'jugement', icon: 'fa-balance-scale', descriptionKey: 'COURSE_DETAIL_ID9.EFFECT_JUDGMENT' }
  ...
  getAccidentSituations(): { id: string; titleKey: string; descriptionKey: string; icon: string }[] {
    ...
    return [
      ...
      { 
        ...
        titleKey: 'COURSE_DETAIL_ID9.ACCIDENT_EXIT_TITLE', 

        descriptionKey: 'COURSE_DETAIL_ID9.ACCIDENT_EXIT_DESC',
      ...
      { 
        ...
        titleKey: 'COURSE_DETAIL_ID9.ACCIDENT_COLLISION_TITLE', 

        descriptionKey: 'COURSE_DETAIL_ID9.ACCIDENT_COLLISION_DESC',
  ...
  getRiskMultipliers(): { substanceKey: string; multiplier: number; color: string }[] {

    return [

      { substanceKey: 'COURSE_DETAIL_ID9.SUBSTANCE_NONE', multiplier: 1, color: '#4caf50' },

      { substanceKey: 'COURSE_DETAIL_ID9.SUBSTANCE_CANNABIS', multiplier: 2, color: '#ff9800' },

      { substanceKey: 'COURSE_DETAIL_ID9.SUBSTANCE_ALCOHOL', multiplier: 3, color: '#f44336' },

      { substanceKey: 'COURSE_DETAIL_ID9.SUBSTANCE_BOTH', multiplier: 14, color: '#b71c1c' }
  ...
  getArmStrengthMessage(): string {
    ...
    return 'COURSE_DETAIL_ID10.ARM_STRENGTH_MESSAGE';
  ...
  getSafetyElementInfo(element: string): { titleKey: string; descriptionKey: string; icon: string } {

    const elements: { [key: string]: { titleKey: string; descriptionKey: string; icon: string } } = {

      'ceinture-avant': {

        titleKey: 'COURSE_DETAIL_ID10.ELEMENT_FRONT_TITLE',

        descriptionKey: 'COURSE_DETAIL_ID10.ELEMENT_FRONT_DESC',
      ...
      'ceinture-arriere': {
        ...
        titleKey: 'COURSE_DETAIL_ID10.ELEMENT_REAR_TITLE',

        descriptionKey: 'COURSE_DETAIL_ID10.ELEMENT_REAR_DESC',
      ...
      'airbag': {
        ...
        titleKey: 'COURSE_DETAIL_ID10.ELEMENT_AIRBAG_TITLE',

        descriptionKey: 'COURSE_DETAIL_ID10.ELEMENT_AIRBAG_DESC',
      ...
      'appuis-tete': {
        ...
        titleKey: 'COURSE_DETAIL_ID10.ELEMENT_HEADREST_TITLE',

        descriptionKey: 'COURSE_DETAIL_ID10.ELEMENT_HEADREST_DESC',
      ...
      'sieges': {
        ...
        titleKey: 'COURSE_DETAIL_ID10.ELEMENT_SEATS_TITLE',

        descriptionKey: 'COURSE_DETAIL_ID10.ELEMENT_SEATS_DESC',
    ...
    return elements[element] || { titleKey: '', descriptionKey: '', icon: '' };
  ...
  startMathQuiz() {
    ...
    this.mathQuizPhase.set('countdown');

    this.mathCountdownValue.set(10);

    

    this.mathCountdownInterval = setInterval(() => {

      const val = this.mathCountdownValue() - 1;

      this.mathCountdownValue.set(val);

      if (val <= 0) {

        clearInterval(this.mathCountdownInterval);

        this.startNextMathTrial();
  ...
  generateMathEquation() {
    ...
    this.mathCurrentEquation.set(`${num1} ${opSymbol} ${num2}`);

    this.mathCorrectAnswer.set(answer);

    this.mathUserAnswer.set(null);

    this.mathFeedback.set(null);

    this.mathStartTime.set(Date.now());
  ...
  startNextMathTrial() {
    ...
    this.mathQuizPhase.set('question');
  ...
  submitMathAnswer() {
    ...
    if (this.mathUserAnswer() === null) return;

    

    const reactionTime = Date.now() - this.mathStartTime();

    const isCorrect = this.mathUserAnswer() === this.mathCorrectAnswer();

    

    if (isCorrect) {

      this.mathScore.update(s => s + 1);

      this.mathFeedback.set({ isCorrect: true, messageKey: 'COURSE_DETAIL_ID7.MATH_CORRECT' });

    } else {

      this.mathFeedback.set({ 

        isCorrect: false, 

        messageKey: 'COURSE_DETAIL_ID7.MATH_INCORRECT', 

        answer: this.mathCorrectAnswer() 
    ...
    this.mathReactionTimes.update(times => [...times, reactionTime]);

    const currentTrial = this.mathTrialCount() + 1;

    this.mathTrialCount.set(currentTrial);



    // If we finished 5 trials

    if (currentTrial >= 5) {

      setTimeout(() => {

        this.mathQuizPhase.set('finished');

      }, 1500);

    } else {

      const delays = [2, 1, 4, 2];

      const nextDelay = delays[currentTrial - 1] || 2;

      

      setTimeout(() => {

        this.mathQuizPhase.set('waiting');

        this.mathTransitionCountdown.set(nextDelay);

        

        this.mathTransitionInterval = setInterval(() => {

          const val = this.mathTransitionCountdown() - 1;

          this.mathTransitionCountdown.set(val);

          if (val <= 0) {

            clearInterval(this.mathTransitionInterval);
  ...
  getAverageMathReactionTime(): number {

    const times = this.mathReactionTimes();
  ...
  resetMathQuiz() {
    ...
    if (this.mathTransitionInterval) {

      clearInterval(this.mathTransitionInterval);

      this.mathTransitionInterval = null;

    }

    this.mathQuizPhase.set('idle');

    this.mathCountdownValue.set(10);

    this.mathTrialCount.set(0);

    this.mathCurrentEquation.set('');

    this.mathCorrectAnswer.set(0);

    this.mathUserAnswer.set(null);

    this.mathReactionTimes.set([]);

    this.mathStartTime.set(0);

    this.mathScore.set(0);

    this.mathFeedback.set(null);

    this.mathTransitionCountdown.set(0);
