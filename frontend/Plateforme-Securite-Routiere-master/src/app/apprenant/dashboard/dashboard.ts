...
export class Dashboard implements OnInit {
  ...
  progressStats: Array<{
    ...
    { labelKey: 'APPRENANT.DASHBOARD.STATS.QCM_PASSED', value: 0, total: 0, icon: 'fas fa-clipboard-check', color: 'success', showRing: false },
    { labelKey: 'APPRENANT.DASHBOARD.STATS.QCM_FAILED', value: 0, total: null, icon: 'fas fa-times-circle', color: 'danger' },
    { labelKey: 'APPRENANT.DASHBOARD.STATS.AVG_SCORE', value: 0, total: 100, icon: 'fas fa-star', color: 'info', suffix: '%', showRing: false },
  ...
  private loadDashboardStats(): void {
    ...
    forkJoin({
      ...
      publishedCourses: this.coursService.listPublished(),
      videos: this.videoService.listPublishedVideos(),
      qcmResults: this.qcmService.getMyResults()
    }).subscribe({
      next: ({ stats, publishedCourses, videos, qcmResults }) => {
        ...
        // Même logique que la liste /apprenant/cours : 11 statiques + cours publiés (hors ids 1-11)
        const publishedDynamic = (publishedCourses || []).filter(
          (c) => !this.staticCourseIds.has(c.id)
        );
        const totalCourses = this.STATIC_COURSES_COUNT + publishedDynamic.length;
        ...
        console.log(`✅ Stats chargées: ${this.STATIC_COURSES_COUNT} cours statiques + ${publishedDynamic.length} dynamiques publiés = ${totalCourses}`);
