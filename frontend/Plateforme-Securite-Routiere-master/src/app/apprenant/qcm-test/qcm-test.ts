...
import { catchError, finalize, Subscription, throwError, timeout } from 'rxjs';
...
export class QcmTest implements OnInit, OnDestroy {
  ...
  private loadQcm(keepSession = false) {
    ...
    const source$ = this.buildQcmSource(lang);

    source$
      .pipe(
        timeout(this.isGenerated ? 120000 : 8000),
        finalize(() => {
          this.loading = false;
      ...
      .subscribe({
        ...
        error: (err) => {
          if (err?.name === 'TimeoutError') {
            this.error = this.translateService.instant('QCM_PANEL.TEST.TIMEOUT_ERROR');
          } else {
            this.error = err.error?.detail || this.translateService.instant('QCM_PANEL.TEST.NOT_FOUND');
          }
          this.cdr.detectChanges();
        }
      });
  ...
  private buildQcmSource(lang: 'fr' | 'ar') {
    if (this.isGenerated) {
      return this.qcmService.getGeneratedQcmForTest(this.qcmId, lang).pipe(
        catchError((err) => {
          if (err?.name === 'TimeoutError') {
    ...
    return this.qcmService.getQcmForTest(this.qcmId).pipe(
      catchError(() => {
