...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, TranslateModule, PlatformLogo],
...
export class Dashboard implements OnInit, AfterViewInit {
  ...
  kpis = signal([
    ...
    { title: 'Total Vidéos', value: 0 as number | string, icon: 'fas fa-film', color: 'info', change: '' }
  ]);
  ...
  navItems: FormateurNavItem[] = [
    { labelKey: 'FORMATEUR.NAV.DASHBOARD', icon: 'fas fa-home', route: '/formateur/dashboard' },
    { labelKey: 'FORMATEUR.NAV.MANAGE_COURSES', icon: 'fas fa-cog', route: '/formateur/cours' },
    { labelKey: 'FORMATEUR.NAV.MANAGE_VIDEOS', icon: 'fas fa-film', route: '/formateur/videos-manage' },
    { labelKey: 'FORMATEUR.NAV.MY_PROFILE', icon: 'fas fa-user', route: '/formateur/profile' }
  ];

  totalPublishedCourses = computed(() => this.mesCours().filter(c => c.status === 'publie').length);
  totalLearnersCount = signal<number>(0);
  totalEnrolledLearners = computed(() => this.totalLearnersCount());
  ...
  // Nombre total de vidéos du formateur connecté
  totalVideosCount = signal<number>(0);
  ...
  isNavItemActive(item: FormateurNavItem): boolean {
    const url = this.router.url;
    const routeActive = url === item.route || url.startsWith(`${item.route}/`);
    if (!item.fragment) {
      return routeActive;
    }

    return routeActive && url.includes(`#${item.fragment}`);
  }
  ...
  private loadDashboardStats(): void {
    this.statisticsService.getFormateurStats().subscribe({
      next: (stats) => {
        ...
        this.totalLearnersCount.set(stats.users?.apprenants ?? 0);
  ...
  private readonly staticFormateurCourses: Cours[] = [
  ...
  mesCours = signal<Cours[]>([...this.staticFormateurCourses]);
  ...
  private loadFormateurCourses(): void {
    ...
    this.coursService.manageList(1, 100).subscribe({
      next: (response) => {
        ...
        const apiIds = new Set(coursFrontend.map(c => c.id));
        const staticCourses = this.staticFormateurCourses.filter(c => !apiIds.has(c.id));
        const merged = [...staticCourses, ...coursFrontend];

        this.mesCours.set(merged);
        this.updateCoursesKpis(merged);
        this.loadingCours.set(false);
      },
      error: (err) => {
        console.error('❌ Erreur chargement backend:', err);
        this.mesCours.set([...this.staticFormateurCourses]);
        this.updateCoursesKpis(this.staticFormateurCourses);
        this.loadingCours.set(false);
      }
    });
  }
  ...
  /**
   * Charge le nombre total de vidéos du formateur connecté.
   */
  private loadTotalVideosCount(): void {
    const currentUser = this.authService.currentUser();
    const formateurId = currentUser?.id;

    if (!formateurId) {
      this.totalVideosCount.set(0);
      return;
    }

    console.log('🔄 Chargement du nombre total de vidéos...');
    this.videoService.countVideos(formateurId).subscribe({
      next: (count) => {
        this.totalVideosCount.set(count);
        const updatedKpis = [...this.kpis()];
        updatedKpis[3].value = count;
        this.kpis.set(updatedKpis);
        console.log('✅ Total vidéos chargées:', count);
      },
      error: (err) => {
  ...
  getCourseCategoryLabel(category: string): string {
    const normalized = this.normalizeTitle(category || '')
      .replace(/[^a-z0-9 ]/g, ' ')
      .replace(/\s+/g, '_')
      .toUpperCase();
  ...
  getCourseTitleLabel(cours: Cours): string {
    const translationKey = `COURSE_DETAIL_ID${cours.id}.COURSE_TITLE`;
    const translated = this.translate.instant(translationKey);
    return translated !== translationKey ? translated : cours.titre;
  }

  getCourseImagePath(cours: Cours): string {
    const mapped = this.imageByTitle[this.normalizeTitle(cours.titre || '')];
    return mapped ? new URL(`assets/cours/${mapped}`, document.baseURI).toString() : '';
  }
  ...
  deleteCours(cours: Cours): void {
    ...
    this.coursService.delete(cours.id).subscribe({
      next: () => {
        const updated = this.mesCours().filter(c => c.id !== cours.id);
        this.mesCours.set(updated);
        this.updateCoursesKpis(updated);
        this.failedImageIds.update((prev) => {
          const next = new Set(prev);
          next.delete(cours.id);
          return next;
        });

        // Synchroniser avec le backend (si la liste venait de l'API)
        this.refreshCourses();
      },
      error: (err) => {
