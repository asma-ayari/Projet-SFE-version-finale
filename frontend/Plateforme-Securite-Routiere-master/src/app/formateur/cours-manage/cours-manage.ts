...
export class CoursManage implements OnInit {
  ...
  getCourseImagePath(course: CourseItem): string {
    ...
    if (mapped) {
      return new URL(`assets/cours/${mapped}`, document.baseURI).toString();
    }

    if (course.image) {
      return new URL(`assets/cours/${course.image}`, document.baseURI).toString();
    }

    if (course.image_url) {
      return this.coursService.resolveImageUrl(course.image_url);
    }
