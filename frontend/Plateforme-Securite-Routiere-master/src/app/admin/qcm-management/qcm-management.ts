...
export class QcmManagement implements OnInit {
  ...
  loadQcms() {
    ...
    this.qcmService.adminListQcms(this.page, this.limit, this.filterCategory || undefined, pub).subscribe({
      ...
      error: (err) => {
        this.error = this.translate.instant('ADMIN.QCM_MANAGEMENT.LOADING_ERROR');
        this.loading = false;
        this.qcms = [];
        this.cdr.markForCheck();
      }
  ...
  togglePublish(qcm: QCMListItem) {
    this.qcmService.togglePublish(qcm.id).subscribe({
      next: (res) => {
        ...
        const statusKey = res.is_published ? 'ADMIN.QCM_MANAGEMENT.QCM_PUBLISHED' : 'ADMIN.QCM_MANAGEMENT.QCM_UNPUBLISHED';
        this.success = `QCM "${qcm.title}" ${this.translate.instant(statusKey)}`;
        this.cdr.markForCheck();
        setTimeout(() => this.success = '', 3000);
      },
      error: () => {
        this.error = this.translate.instant('ADMIN.QCM_MANAGEMENT.PUBLISH_ERROR');
        this.cdr.markForCheck();
      }
    });
  }
  ...
  deleteQcm(qcm: QCMListItem) {
    if (!confirm(`${this.translate.instant('ADMIN.QCM_MANAGEMENT.CONFIRM_DELETE_MSG')} "${qcm.title}" ?`)) return;
    this.qcmService.deleteQcm(qcm.id).subscribe({
      next: () => {
        this.success = this.translate.instant('ADMIN.QCM_MANAGEMENT.QCM_DELETED');
        this.loadQcms();
        setTimeout(() => this.success = '', 3000);
      },
      error: () => {
        this.error = this.translate.instant('ADMIN.QCM_MANAGEMENT.DELETE_ERROR');
        this.cdr.markForCheck();
      }
    });
  }
