...
def apprenant_statistics(
    ...
    published_db_courses = db.query(func.count(Course.id)).filter(Course.is_published == True).scalar() or 0
    # 11 cours statiques intégrés côté frontend + cours dynamiques publiés (hors ids 1-11)
    static_courses_count = 11
    dynamic_published = (
        db.query(func.count(Course.id))
        .filter(Course.is_published == True, Course.id > static_courses_count)
        .scalar()
        or 0
    )
    total_courses_available = static_courses_count + dynamic_published
    ...
    return {
        ...
        "courses": {
            "available": total_courses_available,
            "published_db": published_db_courses,
        },
    }
