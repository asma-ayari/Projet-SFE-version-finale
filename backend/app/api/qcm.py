...
from app.services.qcm_translator import (
    get_source_language,
    translate_list_item,
    translate_public_qcm,
)
...
def _delete_image_file(image_url: Optional[str]) -> None:
    """Supprime le fichier image du disque si c'est un fichier local."""
    if not image_url:
        return
    # Ne pas supprimer les URLs externes (Wikimedia, http...)
...
# Upload image pour une question existante (admin)
...
# Supprimer l'image d'une question (admin)
...
def generate_qcm(
    ...
    qcm = QCM(
        ...
        generation_language=data.language,
    )
    db.add(qcm)
    db.flush()
    for idx, q_data in enumerate(payload.get("questions", []), start=1):
        ...
        question = Question(
            ...
            image_url=q_data.get("image_url"),          # ← MODIFICATION : image générée par le LLM
...
def get_generated_qcm(
    ...
    cache = _category_name_cache(db)
    response = _qcm_to_public_response(qcm, cache)
    if not lang:
        return response
    ...
    try:
        return translate_public_qcm(response, source_lang, lang)
    except Exception as exc:
        _LOGGER.warning("Traduction QCM %s vers %s echouee: %s", qcm_id, lang, exc)
        return response
