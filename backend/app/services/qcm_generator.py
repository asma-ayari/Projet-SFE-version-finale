...
_IMAGE_BANK: dict = {
    ...
    "fin_limitation_vitesse":     "https://upload.wikimedia.org/wikipedia/commons/thumb/3/thirty/Zeichen_278.svg/240px-Zeichen_278.svg.png",
    ...
    "panneau_travaux":            "https://upload.wikimedia.org/wikipedia/commons/thumb/5/53/Zeichen_123.svg/240px-Zeichen_123.svg.png",
    "panneau_dos_ane":            "https://upload.wikimedia.org/wikipedia/commons/thumb/6/69/Zeichen_138.svg/240px-Zeichen_138.svg.png",
    ...
    "arret_interdit":             "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b0/Zeichen_283.10.svg/240px-Zeichen_283.10.svg.png",
    ...
    "sens_giratoire":             "https://upload.wikimedia.org/wikipedia/commons/thumb/2/25/Zeichen_215.svg/240px-Zeichen_215.svg.png",
    "feu_tricolore":              "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a1/Traffic_lights_4_states.svg/60px-Traffic_lights_4_states.svg.png",
    "panneau_autoroute":          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/60/Zeichen_330.1.svg/240px-Zeichen_330.1.svg.png",
    "panneau_parking":            "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b9/Zeichen_314.svg/240px-Zeichen_314.svg.png",
}

_VALID_IMAGE_KEYWORDS = sorted(_IMAGE_BANK.keys())


def _resolve_image_url(keyword: Optional[str]) -> Optional[str]:
    if not keyword:
        return None
    key = keyword.lower().strip().replace(" ", "_").replace("-", "_")
    if key in _IMAGE_BANK:
        return _IMAGE_BANK[key]
    for k in _IMAGE_BANK:
        if k in key or key in k:
            return _IMAGE_BANK[k]
    return None
...
# ──────────────────────────────────────────────────────────────────────────────
# ✅ CORRECTION PRINCIPALE : _build_context
# Le document PDF est indexé en "fr". Pour "ar", on utilise quand même
# ce contexte car Groq traduira le contenu via le prompt arabe.
# ──────────────────────────────────────────────────────────────────────────────
def _build_context(language: str, max_chars: int = _QCM_CONTEXT_MAX_CHARS) -> str:
    ...
    # 1re tentative : chercher des docs dans la langue demandée
    filtered = [
        d for d in docs
    ...
    # ✅ FALLBACK : si aucun doc dans la langue demandée (ex: "ar"),
    # on prend les docs français du même fichier source.
    # Groq génèrera les questions en arabe grâce au prompt.
    if not filtered:
        _LOGGER.warning(
            "Aucun document trouvé pour language='%s', fichier='%s'. "
            "Fallback vers langue 'fr'.",
            language, _QCM_SOURCE_FILE,
        )
        filtered = [
            d for d in docs
            if d.metadata.get("language") == "fr"
            and os.path.splitext((d.metadata.get("source") or "").lower())[0] == expected_base
        ]

    # 2e fallback : si toujours rien, prendre tous les docs du fichier source
    if not filtered:
        _LOGGER.warning(
            "Aucun document trouvé pour fichier='%s'. Fallback vers tous les docs.",
            _QCM_SOURCE_FILE,
        )
        filtered = [
            d for d in docs
            if os.path.splitext((d.metadata.get("source") or "").lower())[0] == expected_base
        ]

    # 3e fallback : prendre n'importe quel document disponible
    if not filtered:
        _LOGGER.warning("Fichier source introuvable. Utilisation de tous les documents disponibles.")
        filtered = docs
...
def _build_prompt(
    ...
    keywords_list = ", ".join(_VALID_IMAGE_KEYWORDS)

    if language == "ar":
        theme_line = f"الموضوع المحدد: {theme}" if theme else ""
        # ✅ CORRECTION : instructions renforcées pour forcer l'arabe
        return f"""أنت خبير في السلامة المرورية في تونس.
⚠️ مهم جداً: يجب أن تكون جميع الأسئلة والإجابات والشرح باللغة العربية الفصحى حصراً.
لا تستخدم اللغة الفرنسية في أي جزء من الإجابة.

اكتب اختبار اختيار من متعدد (QCM) بصيغة JSON فقط وبشكل مضغوط.
المطلوب:
- عدد الأسئلة: {question_count}
- مستوى الصعوبة: {difficulty}
- كل سؤال يحتوي 4 إجابات بالضبط مع إجابة صحيحة واحدة فقط.
- كل سؤال يجب أن يعالج مفهوماً مختلفاً بوضوح.
- تجنب التكرار وإعادة صياغة نفس المفهوم.
- يجب أن يذكر العنوان الموضوع المحدد.
- لا تضف أي نص خارج JSON.
- JSON صارم: علامات اقتباس مزدوجة فقط، بدون فواصل زائدة، ولا أسطر داخل النصوص.
- لكل سؤال أضف "image_keyword": اختر كلمة واحدة من القائمة إذا كانت الصورة مفيدة، وإلا اكتب null.
- الكلمات المتاحة: {keywords_list}
{theme_line}

هيكل JSON المطلوب (بالعربية):
{{
  "title": "عنوان الاختبار بالعربية",
  "description": "وصف مختصر بالعربية",
  "questions": [
    {{
      "text": "نص السؤال بالعربية؟",
      "image_keyword": "panneau_stop",
      "explanation": "شرح مختصر بالعربية",
      "answers": [
        {{"text": "الإجابة الصحيحة بالعربية", "is_correct": true}},
        {{"text": "إجابة خاطئة بالعربية", "is_correct": false}},
        {{"text": "إجابة خاطئة بالعربية", "is_correct": false}},
        {{"text": "إجابة خاطئة بالعربية", "is_correct": false}}
      ]
    }}
...
السياق (استخدمه كمرجع للمعلومات فقط، لكن اكتب الأسئلة بالعربية):
{context}
"""
...
- Pour chaque question, ajoute "image_keyword": choisis UN mot-cle parmi la liste si une image est pertinente, sinon null.
- Mots-cles disponibles: {keywords_list}
{theme_line}
...
def _clean_questions(questions: List[dict]) -> List[dict]:
    ...
    for q in questions:
        ...
        if not any(a.get("is_correct") for a in answers):
            continue
        q["answers"] = answers[:4]
        keyword = q.pop("image_keyword", None)
        q["image_url"] = _resolve_image_url(keyword)
        cleaned.append(q)
    return cleaned
