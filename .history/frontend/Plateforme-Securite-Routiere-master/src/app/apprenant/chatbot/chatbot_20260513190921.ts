...
export class Chatbot implements AfterViewChecked {
  ...
  private applyLocalizedText(): void {
    const welcome = this.translate.instant('CHATBOT.WELCOME');
    const suggestedQuestions = this.languageService.getCurrentLang() === 'ar'
      ? [
          { text: this.translate.instant('CHATBOT.Q_SIGNS'), icon: 'fas fa-traffic-light' },
          { text: this.translate.instant('CHATBOT.Q_PRIORITY'), icon: 'fas fa-car-crash' },
          { text: this.translate.instant('CHATBOT.Q_FIRST_AID'), icon: 'fas fa-tachometer-alt' },
          { text: this.translate.instant('CHATBOT.Q_SPEED'), icon: 'fas fa-ban' }
        ]
      : [
          { text: 'Quelles sont les règles de priorité à un carrefour ?', icon: 'fas fa-traffic-light' },
          { text: "Comment réagir en cas d'accident ?", icon: 'fas fa-car-crash' },
