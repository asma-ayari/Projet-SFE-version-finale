...
export class Chatbot implements AfterViewChecked {
  ...
  messages = signal<Message[]>([
    {
      id: this.getNextId(),
      content: 'Bonjour ! 👋 Je suis votre assistant intelligent spécialisé en sécurité routière en Tunisie. Posez-moi n\'importe quelle question sur le code de la route, envoyez une image de panneau pour l\'identifier, ou utilisez le micro pour parler !',
      sender: 'bot',
      timestamp: new Date()
    }
  ]);
  ...
  ngOnInit() {
    // Use translated welcome message and set recognition/tts language according to current language
    const lang = (inject(LanguageService).getCurrentLang?.() as 'fr' | 'ar') || 'fr';
    // Update recognition and synthesis language if available
    if (this.recognition) {
      this.recognition.lang = lang === 'ar' ? 'ar-SA' : 'fr-FR';
    }
    // Update default bot welcome message from translations
    try {
      const translate = inject(TranslateService);
      const welcome = translate.instant('CHATBOT.DEMO_MSG1') || translate.instant('CHATBOT.WELCOME') || 'Bonjour !';
      this.messages.set([{
        id: this.getNextId(),
        content: welcome,
        sender: 'bot',
