...
const routes: Routes = [
  ...
  /* ─── NOUVEAU : Passage du QCM question par question ─── */
  {
    path: 'qcm-session/:id',
    loadComponent: () => import('./qcm-session/qcm-session.component').then(m => m.QcmSessionComponent)
  },

  /* ─── Résultats d'un QCM (optionnel, si tu veux une page dédiée) ─── */
  {
    path: 'qcm-session/:id/result',
    loadComponent: () => import('./qcm-result/qcm-result').then(m => m.QcmResultComponent)
  },
