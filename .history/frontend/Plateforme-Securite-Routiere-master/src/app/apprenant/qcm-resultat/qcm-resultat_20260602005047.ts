...
export class QcmResultat implements OnInit {
  ...
  private loadQcmDetailAndNormalize() {
    ...
    // Always attempt to load a QCM detail (published first, then generated).
    // Regardless of outcome, we must stop the spinner and display what we can.
    this.qcmService.getQcmForTest(qcmId).pipe(
      ...
      catchError(() => {
        // If both endpoints fail (e.g., unpublished QCM), keep qcmDetail null.
        this.qcmDetail = null;
        return of(null);
      }),
      finalize(() => {
