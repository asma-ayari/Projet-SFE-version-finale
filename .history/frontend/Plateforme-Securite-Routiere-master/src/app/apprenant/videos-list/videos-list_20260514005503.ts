import { Component, OnInit, inject, OnDestroy, ChangeDetectorRef, NgZone, ChangeDetectionStrategy } from '@angular/core';
    ...
    this.videoService
      ...
      .pipe(
        ...
        finalize(() => {
          this.ngZone.run(() => {
            this.loading = false;
            console.log('✅ Loading complete. Videos found:', this.videos.length);
        ...
        next: (videos) => {
          this.ngZone.run(() => {
            ...
            this.videos = Array.isArray(videos)
              ...
              : [];
            console.log('📊 After sorting:', this.videos);
            this.cdr.markForCheck();
          });
        },
        error: (err) => {
