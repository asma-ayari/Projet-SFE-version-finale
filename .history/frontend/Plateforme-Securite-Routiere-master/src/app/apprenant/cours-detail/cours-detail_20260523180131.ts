...
export class CoursDetail implements OnInit {
    ...
    if (id) {
      ...
      courseRequest.pipe(
        ...
        catchError((err) => {
          ...
          // En cas d'erreur 404, rediriger vers la liste des cours
          ...
          if (err.status === 404) {
            ...
            // Pour autres erreurs, afficher un message
  ...
  getFactorsAffectingReaction(): { factor: string; icon: string; effect: string }[] {
    ...
    return [
      ...
      { factor: 'الإرهاق', icon: 'fa-bed', effect: 'يزيد من وقت رد الفعل' },

      { factor: 'الكحول', icon: 'fa-wine-glass', effect: 'يبطئ ردود الأفعال' },

      { factor: 'الهاتف', icon: 'fa-mobile-alt', effect: 'يشتت الانتباه' },

      { factor: 'الأدوية', icon: 'fa-pills', effect: 'قد تؤثر على اليقظة' },

      { factor: 'العمر', icon: 'fa-user-clock', effect: 'تتناقص ردود الأفعال مع التقدم في العمر' }
  ...
  getCannabisEffects(): { effect: string; icon: string; description: string }[] {
    ...
    return [
      ...
      { effect: 'perception', icon: 'fa-eye-slash', description: 'اضطراب الإدراك البصري والمكاني' },

      { effect: 'attention', icon: 'fa-brain', description: 'انخفاض الانتباه والتركيز' },

      { effect: 'reaction', icon: 'fa-clock', description: 'زيادة زمن الاستجابة' },

      { effect: 'coordination', icon: 'fa-hands', description: 'اضطرابات في التنسيق الحركي' },

      { effect: 'jugement', icon: 'fa-balance-scale', description: 'اضطراب الحكم واتخاذ المخاطر' }
  ...
  getAccidentSituations(): { id: string; title: string; description: string; icon: string }[] {
    ...
    return [
      ...
      { 
        ...
        title: 'خروج عن الطريق', 

        description: 'صعوبة الحفاظ على المسار، خاصة في المنعطفات. يستهين السائق بالمنعطف أو يتأخر في التفاعل.',
      ...
      { 
        ...
        description: 'إدراك مضطرب للمسافات والسرعة. لا يكتشف السائق في الوقت المناسب عائقًا ثابتًا أو مركبة.',
  ...
  getRiskMultipliers(): { substance: string; multiplier: number; color: string }[] {
    ...
    return [
      ...
      { substance: 'لا شيء', multiplier: 1, color: '#4caf50' },

      { substance: 'القنب وحده', multiplier: 2, color: '#ff9800' },

      { substance: 'الكحول وحده (0.5 غ/ل)', multiplier: 3, color: '#f44336' },

      { substance: 'القنب + الكحول', multiplier: 14, color: '#b71c1c' }
  ...
  getSafetyElementInfo(element: string): { title: string; description: string; icon: string } {
    ...
    const elements: { [key: string]: { title: string; description: string; icon: string } } = {
      ...
      'ceinture-avant': {
        ...
        description: 'يثبّت الجسم عند الاصطدام ويوزّع القوى على الأجزاء القوية من الجسم (الحوض، الصدر). وهو مزود بشدّاد يربطه عند وقوع الصدمة.',
      ...
      'ceinture-arriere': {
        ...
        description: 'لا يقل أهمية عن الحزام الأمامي. الراكب الخلفي غير المربوط يتحول إلى جسم قاتل للركاب الأماميين عند الاصطدام.',
      ...
      'airbag': {
        ...
        description: 'تعمل خلال أجزاء من الألف من الثانية عند الاصطدام لتخفيف صدمة الرأس والصدر. وتعمل كمكمّل للحزام، وليس وحدها أبدًا.',
      ...
      'appuis-tete': {
        ...
        description: 'تحمي فقرات الرقبة من إصابة "الارتداد العنقي" عند الصدمة الخلفية. ويجب ضبطها على مستوى العينين أو أعلى الجمجمة.',
      ...
      'sieges': {
        ...
        description: 'مصممة لامتصاص جزء من طاقة الصدمة والحفاظ على الركاب في وضعهم. ويجب أن يتيح مقعد السائق الوصول إلى الدواسات مع ثني الذراعين قليلًا.',
