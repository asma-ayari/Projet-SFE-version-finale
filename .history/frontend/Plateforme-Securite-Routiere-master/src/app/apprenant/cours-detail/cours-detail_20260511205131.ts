...
export class CoursDetail implements OnInit {
  ...
  getRoadConditionLabel(): string {
    switch (this.selectedRoadCondition()) {
      case 'wet': return 'Chaussée mouillée';
      case 'soaked': return 'Chaussée détrempée';
      default: return 'Chaussée sèche';
    }
  }
  ...
  getVisualFieldDescription(): string {
    const speed = this.visualFieldSpeed();
    if (speed <= 40) {
      return 'Vision périphérique large, perception optimale des dangers latéraux';
    }
    if (speed <= 70) {
      return 'Vision périphérique réduite, attention aux véhicules sur les côtés';
    }
    if (speed <= 100) {
      return 'Vision focalisée vers l\'avant, danger de ne pas percevoir les côtés';
    }
    return 'Vision tunnel, perception latérale très limitée';
  }

  getSpeedLimitForEnvironment(): { environment: string; limit: number; reason: string }[] {
    return [
      { environment: 'Ville', limit: 50, reason: 'Environnement complexe, piétons, intersections' },
      { environment: 'Route', limit: 80, reason: 'Moins de complexité mais vigilance nécessaire' },
      { environment: 'Voie rapide', limit: 110, reason: 'Circulation fluide, moins d\'intersections' },
      { environment: 'Autoroute', limit: 130, reason: 'Voies séparées, pas de piétons' }
    ];
  }
  ...
  getDrinkEquivalences(): { name: string; description: string; volume: string; degree: string; doses: number }[] {
    return [
      { name: 'Bière légère', description: 'Cannette de bière', volume: '33 cl', degree: '5.9°', doses: 2 },
      { name: 'Bière forte', description: 'Cannette de bière forte', volume: '50 cl', degree: '10°', doses: 4 },
      { name: 'Alcopop', description: 'Cannette de cocktail', volume: '25 cl', degree: '5°', doses: 1 },
      { name: 'Vin', description: 'Bouteille de vin', volume: '75 cl', degree: '12°', doses: 7 },
      { name: 'Whisky', description: 'Verre bien servi', volume: '9 cl', degree: '40°', doses: 3 }
    ];
  }
  ...
  getBrainProcessingSteps(): { step: number; icon: string; title: string; description: string }[] {
    return [
      { step: 1, icon: 'fa-eye', title: 'Perception', description: 'Nos yeux, nos oreilles captent des stimulus provenant de l\'environnement.' },
      { step: 2, icon: 'fa-brain', title: 'Analyse', description: 'Ces messages sont analysés, reconnus et interprétés.' },
      { step: 3, icon: 'fa-hand-pointer', title: 'Décision', description: 'Nous décidons de la réponse à donner et transmettons les ordres pour agir.' }
    ];
  }
