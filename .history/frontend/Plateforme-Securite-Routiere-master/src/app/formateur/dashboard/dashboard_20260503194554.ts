...
export class Dashboard implements OnInit, AfterViewInit {
  ...
  // QCM du formateur
  qcmsFormateur = signal<QCMListItem[]>([]);
  loadingQcms = signal(false);
  ...
  navItems: FormateurNavItem[] = [
    ...
    { label: 'Statistiques', icon: 'fas fa-chart-line', route: '/formateur/dashboard', fragment: 'formateur-stats' },
  ...
  createdQuizCount = computed(() => this.qcmsFormateur().length);
  ...
  /**
   * Charge les QCM publiés associés aux formations du formateur
   */
  private loadFormateurQcms(): void {
    this.loadingQcms.set(true);
    console.log('🔄 Chargement des QCM du formateur...');

    this.qcmService.getFormateurQcms().subscribe({
      next: (qcms) => {
        console.log('✅ QCM chargés:', qcms.length);
        this.qcmsFormateur.set(qcms);
        this.loadingQcms.set(false);
      },
      error: (err) => {
        console.error('❌ Erreur chargement des QCM:', err);
        this.loadingQcms.set(false);
      }
    });
  }
  ...
  /**
   * Rafraîchit les QCM depuis le backend
   */
  refreshQcms(): void {
    console.log('🔄 Rafraîchissement des QCM...');
    this.loadFormateurQcms();
  }
  ...
  /**
   * Voir les détails et statistiques d'un QCM
   */
  viewQcmDetails(qcmId: number): void {
    console.log('👁️ Affichage des détails du QCM (formateur):', qcmId);
    // ✅ SÉCURISÉ: Route dédiée au formateur
    this.router.navigate(['/formateur/qcm', qcmId]);
  }
}
