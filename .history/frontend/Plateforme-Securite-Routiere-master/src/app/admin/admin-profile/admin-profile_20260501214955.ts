...
export class AdminProfile implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Init shared avatar state and subscribe to updates
    this.avatarService.initForCurrentUser();
    this.avatarService.avatarUrl$.pipe(takeUntil(this.destroy$)).subscribe((url) => {
      this.profilePhotoPreview.set(url || null);
    });
  }
  ...
  private handleFileSelection(file: File): void {
    ...
    if (!validation.valid) {
      this.showToast('error', validation.error || 'Fichier invalide');
      return;
    }
    ...
    this.avatarService.getImagePreview(file).then(preview => {
      ...
      this.showToast('error', 'Impossible de générer l\'aperçu');
    });
  }
    ...
    this.avatarService.uploadAvatar(file)
      ...
      .subscribe({
        next: (response) => {
          ...
          this.showToast('success', 'Photo mise à jour ✓');
        },
        error: (error) => {
          ...
          this.showToast('error', 'Erreur lors du téléchargement de la photo');
        }
      });
  }
    ...
    this.avatarService.deleteAvatar()
      ...
      .subscribe({
        next: () => {
          ...
          this.authService.setCurrentUser(updatedUser);
          this.profilePhotoPreview.set(null);
          this.showToast('success', 'Photo supprimée ✓');
        },
        error: (error) => {
    ...
    this.http.put<UserResponse>(`${this.apiUrl}/profile`, formData)
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.showToast('error', error.error?.detail || 'Erreur lors de la mise à jour du profil');
        }
      });
  }
  ...
  changePassword(): void {
    ...
    this.http.put(`${this.apiUrl}/change-password`, passwordData)
      ...
      .subscribe({
        next: () => {
          this.showToast('success', 'Mot de passe changé avec succès ✓');
          this.passwordForm.reset();
        },
        error: (error) => {
          console.error('Password change error:', error);
          this.showToast('error', error.error?.detail || 'Erreur lors du changement de mot de passe');
        }
      });
  }
