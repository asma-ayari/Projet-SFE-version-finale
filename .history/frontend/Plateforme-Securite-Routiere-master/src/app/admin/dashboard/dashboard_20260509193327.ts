...
export class Dashboard implements OnInit {
  ...
  kpis = [
    ...
    {
      titleKey: 'DASHBOARD.COURSES_PUBLISHED',
      value: 0 as number | string,
      change: '',
      trend: 'up',
    ...
    {
      titleKey: 'DASHBOARD.QCM_COMPLETED',
      value: 0 as number | string,
      change: '',
      trend: 'up',
    ...
    {
      titleKey: 'DASHBOARD.SUCCESS_RATE',
      value: '0%' as number | string,
      change: '',
      trend: 'up',
  ...
  private syncLanguageState(): void {
    this.currentDateLocale = this.languageService.getCurrentLang();
  }
  ...
  private loadDashboardData(): void {
    ...
    this.statisticsService.getAdminStats().subscribe({
      ...
      error: (err) => {
        this.error.set(this.translate.instant('DASHBOARD.STATS_ERROR'));
        this.loadingCharts.set(false);
        this.cdr.detectChanges();
      }
    });
    ...
    this.statisticsService.getQCMPassRates().subscribe({
      next: (response) => {
        ...
        this.qcmChartData = {
          ...
          datasets: [
            {
              label: this.translate.instant('DASHBOARD.QCM_PASS_RATE_LABEL'),
              data: data,
              backgroundColor: [
                'rgba(21, 101, 192, 0.8)',
                'rgba(46, 125, 50, 0.8)',
    ...
    this.statisticsService.getUserRegistrations(this.registrationPeriod()).subscribe({
      next: (response) => {
        ...
        this.usersChartData = {
          ...
          datasets: [
            {
              label: this.translate.instant('DASHBOARD.NEW_USERS_LABEL'),
              data: data,
              borderColor: '#1565c0',
  ...
  // Quick actions
  quickActions = [
    { labelKey: 'DASHBOARD.CREATE_QCM', icon: 'fas fa-clipboard-list', route: '/admin/qcm/create', color: 'btn-success' },
    { labelKey: 'DASHBOARD.MANAGE_USERS', icon: 'fas fa-user-cog', route: '/admin/users', color: 'btn-warning' },
    { labelKey: 'DASHBOARD.VIEW_DOCUMENTS', icon: 'fas fa-file-pdf', route: '/admin/documents', color: 'btn-info' }
  ];
  ...
  private applyChartTranslations(): void {
    ...
    this.userTypesChartData = {
      ...this.userTypesChartData,
      labels: [
        this.translate.instant('DASHBOARD.USER_TYPE_LEARNERS'),
        this.translate.instant('DASHBOARD.USER_TYPE_TRAINERS'),
        this.translate.instant('DASHBOARD.USER_TYPE_ADMINS')
      ]
    };
  }
}
