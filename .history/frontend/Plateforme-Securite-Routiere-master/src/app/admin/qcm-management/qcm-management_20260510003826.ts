...
export class QcmManagement implements OnInit {
  ...
  togglePublish(qcm: QCMListItem) {
    this.qcmService.togglePublish(qcm.id).subscribe({
      next: (res) => {
        ...
        const statusKey = res.is_published ? 'QCM_MANAGEMENT.QCM_PUBLISHED' : 'QCM_MANAGEMENT.QCM_UNPUBLISHED';
  ...
  deleteQcm(qcm: QCMListItem) {
    if (!confirm(`${this.translate.instant('QCM_MANAGEMENT.CONFIRM_DELETE_MSG')} "${qcm.title}" ?`)) return;
    this.qcmService.deleteQcm(qcm.id).subscribe({
      next: () => {
        this.success = this.translate.instant('QCM_MANAGEMENT.QCM_DELETED');
        this.loadQcms();
        setTimeout(() => this.success = '', 3000);
      },
      error: () => {
