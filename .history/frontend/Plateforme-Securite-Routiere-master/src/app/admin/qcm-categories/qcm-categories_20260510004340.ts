...
export class QcmCategories implements OnInit {
    ...
    loadCategories() {
        ...
        this.qcmService.listCategories().subscribe({
            ...
            error: (err) => {
                this.error = this.translate.instant('ADMIN.QCM_CATEGORIES.LOADING_ERROR');
                this.loading = false;
                this.cdr.markForCheck();
            }
        });
    ...
    createCategory() {
        ...
        if (!name) {
            this.error = this.translate.instant('ADMIN.QCM_CATEGORIES.NAME_REQUIRED');
            return;
        }
        ...
        this.qcmService.createCategory(name).subscribe({
            next: () => {
                ...
                this.creating = false;
                this.success = this.translate.instant('ADMIN.QCM_CATEGORIES.CATEGORY_ADDED');
                this.loadCategories();
                setTimeout(() => this.success = '', 3000);
            },
            error: (err) => {
                this.error = this.translate.instant('ADMIN.QCM_CATEGORIES.CREATE_ERROR');
                this.creating = false;
                this.cdr.markForCheck();
            }
        });
    ...
    saveEdit(cat: QCMCategory) {
        ...
        this.qcmService.updateCategory(cat.id, { name, is_active: cat.is_active }).subscribe({
            next: (updated) => {
                ...
                this.cancelEdit();
                this.success = this.translate.instant('ADMIN.QCM_CATEGORIES.CATEGORY_MODIFIED');
                this.cdr.markForCheck();
                setTimeout(() => this.success = '', 3000);
            },
            error: (err) => {
                this.error = this.translate.instant('ADMIN.QCM_CATEGORIES.UPDATE_ERROR');
                this.cdr.markForCheck();
            }
        });
    }
    ...
    toggleActive(cat: QCMCategory) {
        ...
        this.qcmService.updateCategory(cat.id, { name: cat.name, is_active: !cat.is_active }).subscribe({
            next: (updated) => {
                this.categories = this.categories.map(c => c.id === cat.id ? updated : c);
                const msgKey = updated.is_active ? 'ADMIN.QCM_CATEGORIES.CATEGORY_ACTIVATED' : 'ADMIN.QCM_CATEGORIES.CATEGORY_DEACTIVATED';
    ...
    deleteCategory(cat: QCMCategory) {
        if (!confirm(`${this.translate.instant('ADMIN.QCM_CATEGORIES.CONFIRM_DELETE_MSG')} "${cat.name}" ?`)) return;

        this.error = '';
        this.qcmService.deleteCategory(cat.id).subscribe({
            next: () => {
                this.categories = this.categories.filter(c => c.id !== cat.id);
                this.success = this.translate.instant('ADMIN.QCM_CATEGORIES.CATEGORY_DELETED');
                this.cdr.markForCheck();
                setTimeout(() => this.success = '', 3000);
            },
            error: (err) => {
                this.error = this.translate.instant('ADMIN.QCM_CATEGORIES.DELETE_ERROR');
                this.cdr.markForCheck();
            }
        });
    }
