...
# Chemins Swagger/ReDoc — CSP assouplie pour permettre le chargement
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # ✅ CSP différente selon le chemin
        if request.url.path in _DOCS_PATHS:
            # Swagger UI et ReDoc ont besoin de cdn.jsdelivr.net
            response.headers["Content-Security-Policy"] = (
                "default-src 'self'; "
                "script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://accounts.google.com; "
                "style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; "
                "img-src 'self' data: https://*.googleusercontent.com https://fastapi.tiangolo.com; "
                "font-src 'self' https://cdn.jsdelivr.net; "
                "connect-src 'self'; "
                "frame-src https://accounts.google.com; "
            )
        else:
            # CSP stricte pour toutes les autres routes
            response.headers["Content-Security-Policy"] = (
                "default-src 'self'; "
