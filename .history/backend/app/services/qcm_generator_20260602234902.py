...
# ──────────────────────────────────────────────────────────────────────────────
# BANQUE D'IMAGES : mot-clé → URL image Wikimedia (panneaux tunisiens / européens)
# ──────────────────────────────────────────────────────────────────────────────
_IMAGE_BANK: dict = {
    # Priorité
...
# Liste des mots-clés exposés au LLM dans le prompt
_VALID_IMAGE_KEYWORDS = sorted(_IMAGE_BANK.keys())


def _resolve_image_url(keyword: Optional[str]) -> Optional[str]:
    """Convertit un keyword renvoyé par le LLM en URL réelle.
    Tolère les petites variantes (tirets, majuscules, préfixe/suffixe partiel).
    """
    if not keyword:
        return None
    ...
    # Correspondance partielle : le keyword contient ou est contenu dans une clé
...
def _build_prompt(
    ...
    # Liste des mots-clés disponibles pour les images
...
- لكل سؤال أضف "image_keyword": اختر كلمة واحدة من القائمة إذا كانت الصورة مفيدة لفهم السؤال، وإلا اكتب null.
...
- Pour chaque question, ajoute "image_keyword": choisis UN mot-cle parmi la liste si une image est pertinente pour comprendre la question, sinon null.
...
def _clean_questions(questions: List[dict]) -> List[dict]:
    """Valide, résout les images et retourne les questions nettoyées."""
    cleaned = []
    for q in questions:
...
def generate_qcm_from_docs(
    ...
    if len(collected) < question_count:
        ...
        # ── Nettoyage + résolution image (fallback) ───────────────────────────
        cleaned = _clean_questions(questions)
        # ─────────────────────────────────────────────────────────────────────
